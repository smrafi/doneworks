...
class TellMeMdModelCategory extends JModel
{
    ...
    function getPostData()
    {
        ...
        $this->_data->cat_name = JRequest::getVar('cat_name', '');
        $this->_data->cat_description = JRequest::getVar('cat_description', '');
    ...
    function validate()
    {
        if($this->_data->cat_name == '')
        {
            $this->_app->enqueueMessage(JText::_('COM_TELLMEMD_CATNAME_NOT_FILLED_ERROR'),'error');
            return false;
        }
        
        if($this->_data->cat_description == '')
        {
            $this->_app->enqueueMessage(JText::_('COM_TELLMEMD_CATDESC_NOT_FILLED_ERROR'),'error');
            return false;
        }
	...
	$limitstart = ($limit != 0 ? (floor($limitstart / $limit) * $limit) : 0); // In case limit has been changed
        ...
        $query_cols = "Select cats.* ";
        $query_from = "From ".TABLE_PREFIX."category As cats ";
        $query_where = "";
        
        if($filter_var)
            $query_where .= "Where cats.cat_description Like '%$filter_var%' ";
        
        $query_order = "Order By cats.cat_name ";
    ...
    function getCatArrayList($option = '')
    {
        $query = "Select id, cat_name From ".TABLE_PREFIX."category Where published = 1";
        $this->_db->setQuery($query);
	$catlist = $this->_db->loadObjectList();
        ...
        if($option)
            $catlist_array[0] = $option;
        foreach($catlist as $cat)
            $catlist_array[$cat->id] = $cat->cat_name;
        
        return $catlist_array;;
    }
