		...
		*/
		...
		$select.find('option').each(function(i){
			...
			if(value.length > 0){
				...
				html.push('<label for="'+inputID+'" class="'+labelClasses.join(' ')+'"><input id="'+inputID+'" style="background: '+title+' !important;" type="'+(o.multiple ? 'checkbox' : 'radio')+'" name="'+select.name+'" value="'+value+'" title="'+title+'"');
				if($this.is(':selected')){
				...
				html.push('/><font style="text-align:right; background: '+title+' !important;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font>&nbsp;'+title+'</label></li>');
			}
		});
		html.push('</ul></div>');
