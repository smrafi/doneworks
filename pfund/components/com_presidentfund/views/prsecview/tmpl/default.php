...
<h1>Application Data Review - Secretary President Fund</h1>
...
<div class="comp-content">
    <form class="submit-form" action="index.php" method="post" name="adminForm">
        ...
        <input type="hidden" name="controller" value="prsec" />
        ...
        <div id="grant-input">
            <?php
            if($this->application_data->application_type != APPLICATION_TYPE_REIMBURSMENT)
            {
                ?>
            <div id="amount-input">
                <span>Senior Assistant Secretary (PF)</span><br/><br/>
                A grant of Rs. <?php echo number_format($this->sas_data->sas_grant, 2, '.', ','); ?> may be considered pl.
            </div>
            <input type="hidden" name="sec_grant" id="sec_grant" value="<?php echo $this->sas_data->sas_grant; ?>" />
            <?php
            }
            else
            {
                ?>
            <div id="amount-input">
                <span>Account Head (PF)</span><br/><br/>
                A grant of Rs. <?php echo number_format($this->sas_data->account_grant, 2, '.', ','); ?> may be considered pl.
            </div>
            <input type="hidden" name="sec_grant" id="sec_grant" value="<?php echo $this->sas_data->account_grant; ?>" />
            <?php
            }
            ?>
            <div id="note-input">
                <span>Comment:</span><br/>
                <textarea cols="30" rows="5" name="sec_comment" id="sec_comment"></textarea>
            </div>
        </div>
    </form>
</div>